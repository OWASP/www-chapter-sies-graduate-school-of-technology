---
layout: col-sidebar
title: OWASP SIES Graduate School of Technology
tags: owasp-siesgst
region: Asia
country: India
meetup-group: 
---

## Welcome
Welcome to the official page of SIES Graduate School of Technology's student chapter of OWASP! Founded in August 2025, our chapter is dedicated to helping students explore, learn, and grow in the exciting world of cybersecurity. Whether you’re a total beginner or already a tech enthusiast, there’s a place for you here. Our mission is simple: **make learning security fun, interactive, and impactful.**

## Participation
All events hosted by OWASP SIES GST are open to everyone. Students, faculty, and anyone curious about cybersecurity. Get involved in our **CTF competitions, technical workshops, projects, and other hands-on activities** that will sharpen your skills, boost your confidence, and connect you with like-minded peers. No prior experience is required; all that matters is your curiosity and willingness to learn.

## Next Meeting/Event

### OWASP SIES GST CTF Workshop
**Date:** 2 March 2026  
**Type:** Hands-on Workshop  
**Mode:** Offline  
**Description:**  
This workshop is designed to prepare participants for the upcoming Capture The Flag competition. We will cover CTF basics, common challenge types such as Web, Linux, and OSINT, and walk through live problem-solving techniques. No prior experience is required.

---

### OWASP SIES GST Capture The Flag (CTF)

<img src="/assets/images/CTF-5-7.png" alt="OWASP SIES GST CTF Poster" style="max-width: 600px; width: 100%; height: auto; display: block; margin: 20px auto;">

**Date:** 5 March 2026 – 7 March 2026  
**Type:** Competitive CTF Event  
**Mode:** Hybrid  
**Description:**  
A multi-day Capture The Flag competition focused on real-world cybersecurity challenges. Participants will work individually or in teams to solve challenges related to Web Security, Cryptography, Linux, and Basic Forensics. Certificates and recognition will be provided to top performers.
